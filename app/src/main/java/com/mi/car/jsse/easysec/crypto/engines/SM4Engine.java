...
public class SM4Engine implements BlockCipher {
    ...
    private int tau(int A) {
        int b0 = Sbox[(A >> 24) & GF2Field.MASK] & 255;
        int b1 = Sbox[(A >> 16) & GF2Field.MASK] & 255;
        int b2 = Sbox[(A >> 8) & GF2Field.MASK] & 255;
        return (b0 << 24) | (b1 << 16) | (b2 << 8) | (Sbox[A & GF2Field.MASK] & 255);
    }
    ...
    private int[] expandKey(boolean forEncryption, byte[] key) {
        ...
        if (forEncryption) {
            rk2[0] = K[0] ^ T_ap(((K[1] ^ K[2]) ^ K[3]) ^ CK[0]);
            rk2[1] = K[1] ^ T_ap(((K[2] ^ K[3]) ^ rk2[0]) ^ CK[1]);
            rk2[2] = K[2] ^ T_ap(((K[3] ^ rk2[0]) ^ rk2[1]) ^ CK[2]);
            rk2[3] = K[3] ^ T_ap(((rk2[0] ^ rk2[1]) ^ rk2[2]) ^ CK[3]);
            for (int i = 4; i < 32; i++) {
                rk2[i] = rk2[i - 4] ^ T_ap(((rk2[i - 3] ^ rk2[i - 2]) ^ rk2[i - 1]) ^ CK[i]);
            }
        } else {
            rk2[31] = K[0] ^ T_ap(((K[1] ^ K[2]) ^ K[3]) ^ CK[0]);
            rk2[30] = K[1] ^ T_ap(((K[2] ^ K[3]) ^ rk2[31]) ^ CK[1]);
            rk2[29] = K[2] ^ T_ap(((K[3] ^ rk2[31]) ^ rk2[30]) ^ CK[2]);
            rk2[28] = K[3] ^ T_ap(((rk2[31] ^ rk2[30]) ^ rk2[29]) ^ CK[3]);
            for (int i2 = 27; i2 >= 0; i2--) {
                rk2[i2] = rk2[i2 + 4] ^ T_ap(((rk2[i2 + 3] ^ rk2[i2 + 2]) ^ rk2[i2 + 1]) ^ CK[31 - i2]);
            }
        }
        return rk2;
    }
    ...
    private int F0(int[] X2, int rk2) {
        return X2[0] ^ T(((X2[1] ^ X2[2]) ^ X2[3]) ^ rk2);
    }

    private int F1(int[] X2, int rk2) {
        return X2[1] ^ T(((X2[2] ^ X2[3]) ^ X2[0]) ^ rk2);
    }

    private int F2(int[] X2, int rk2) {
        return X2[2] ^ T(((X2[3] ^ X2[0]) ^ X2[1]) ^ rk2);
    }

    private int F3(int[] X2, int rk2) {
        return X2[3] ^ T(((X2[0] ^ X2[1]) ^ X2[2]) ^ rk2);
    }
