...
public class QTesla3p {
    ...
    static int generateKeyPair(byte[] publicKey, byte[] privateKey, SecureRandom secureRandom) {
        ...
        byte[] randomnessExtended = new byte[256];
        long[] secretPolynomial = new long[2048];
        long[] errorPolynomial = new long[10240];
        long[] A = new long[10240];
        long[] T = new long[10240];
        long[] s_ntt = new long[2048];
        secureRandom.nextBytes(randomness);
        HashUtils.secureHashAlgorithmKECCAK256(randomnessExtended, 0, 256, randomness, 0, 32);
        for (int k = 0; k < 5; k++) {
            do {
                nonce++;
                Gaussian.sample_gauss_poly(nonce, randomnessExtended, k * 32, errorPolynomial, k * 2048);
            } while (checkPolynomial(errorPolynomial, k * 2048, 901));
        }
        do {
            nonce++;
            Gaussian.sample_gauss_poly(nonce, randomnessExtended, 160, secretPolynomial, 0);
        } while (checkPolynomial(secretPolynomial, 0, 901));
        QTesla3PPolynomial.poly_uniform(A, randomnessExtended, BERTags.PRIVATE);
        QTesla3PPolynomial.poly_ntt(s_ntt, secretPolynomial);
        for (int k2 = 0; k2 < 5; k2++) {
            QTesla3PPolynomial.poly_mul(T, k2 * 2048, A, k2 * 2048, s_ntt);
            QTesla3PPolynomial.poly_add_correct(T, k2 * 2048, T, k2 * 2048, errorPolynomial, k2 * 2048);
        }
        encodePublicKey(publicKey, T, randomnessExtended, BERTags.PRIVATE);
        encodePrivateKey(privateKey, secretPolynomial, errorPolynomial, randomnessExtended, BERTags.PRIVATE, publicKey);
        return 0;
    }
    ...
    static int generateSignature(byte[] signature, byte[] message, int messageOffset, int messageLength, byte[] privateKey, SecureRandom secureRandom) {
        ...
        int[] pos_list = new int[40];
        short[] sign_list = new short[40];
        long[] y = new long[2048];
        long[] y_ntt = new long[2048];
        long[] Sc = new long[2048];
        long[] z = new long[2048];
        long[] v = new long[10240];
        long[] Ec = new long[10240];
        long[] a = new long[10240];
        int nonce = 0;
        boolean rsp = false;
        System.arraycopy(privateKey, 12320, randomness_input, 0, 32);
        byte[] tmp = new byte[32];
        secureRandom.nextBytes(tmp);
        System.arraycopy(tmp, 0, randomness_input, 32, 32);
        HashUtils.secureHashAlgorithmKECCAK256(randomness_input, 64, 40, message, 0, messageLength);
        HashUtils.secureHashAlgorithmKECCAK256(randomness, 0, 32, randomness_input, 0, randomness_input.length - 40);
        System.arraycopy(privateKey, 12352, randomness_input, randomness_input.length - 40, 40);
        QTesla3PPolynomial.poly_uniform(a, privateKey, 12288);
        while (true) {
            nonce++;
            sample_y(y, randomness, 0, nonce);
            QTesla3PPolynomial.poly_ntt(y_ntt, y);
            for (int k = 0; k < 5; k++) {
                QTesla3PPolynomial.poly_mul(v, k * 2048, a, k * 2048, y_ntt);
            }
            ...
            if (!testRejection(z)) {
                for (int k2 = 0; k2 < 5; k2++) {
                    QTesla3PPolynomial.sparse_mul8(Ec, k2 * 2048, privateKey, (k2 + 1) * 2048, pos_list, sign_list);
                    QTesla3PPolynomial.poly_sub(v, k2 * 2048, v, k2 * 2048, Ec, k2 * 2048);
                    rsp = test_correctness(v, k2 * 2048);
                    if (rsp) {
                        break;
                    }
                }
    ...
    static int verifying(byte[] message, byte[] signature, int signatureOffset, int signatureLength, byte[] publicKey) {
        ...
        int[] pos_list = new int[40];
        short[] sign_list = new short[40];
        int[] pk_t = new int[10240];
        long[] w = new long[10240];
        long[] a = new long[10240];
        long[] Tc = new long[10240];
        long[] z = new long[2048];
        long[] z_ntt = new long[2048];
        ...
        HashUtils.secureHashAlgorithmKECCAK256(hm, 0, 40, message, 0, message.length);
        HashUtils.secureHashAlgorithmKECCAK256(hm, 40, 40, publicKey, 0, 38400);
        QTesla3PPolynomial.poly_uniform(a, seed, 0);
        encodeC(pos_list, sign_list, c, 0);
        QTesla3PPolynomial.poly_ntt(z_ntt, z);
        for (int k = 0; k < 5; k++) {
            QTesla3PPolynomial.sparse_mul32(Tc, k * 2048, pk_t, k * 2048, pos_list, sign_list);
            QTesla3PPolynomial.poly_mul(w, k * 2048, a, k * 2048, z_ntt);
            QTesla3PPolynomial.poly_sub(w, k * 2048, w, k * 2048, Tc, k * 2048);
        }
        hashFunction(c_sig, 0, w, hm, 0);
        if (!memoryEqual(c, 0, c_sig, 0, 32)) {
            return -3;
        }
        return 0;
    }

    static void encodePrivateKey(byte[] privateKey, long[] secretPolynomial, long[] errorPolynomial, byte[] seed, int seedOffset, byte[] publicKey) {
        for (int i = 0; i < 2048; i++) {
            privateKey[0 + i] = (byte) ((int) secretPolynomial[i]);
        }
        int skPtr = 0 + 2048;
        for (int k = 0; k < 5; k++) {
            for (int i2 = 0; i2 < 2048; i2++) {
                privateKey[(k * 2048) + i2 + 2048] = (byte) ((int) errorPolynomial[(k * 2048) + i2]);
            }
        }
        int skPtr2 = skPtr + 10240;
        System.arraycopy(seed, seedOffset, privateKey, skPtr2, 64);
        int skPtr3 = skPtr2 + 64;
        HashUtils.secureHashAlgorithmKECCAK256(privateKey, skPtr3, 40, publicKey, 0, 38400);
        int skPtr4 = skPtr3 + 40;
    }

    static void encodePublicKey(byte[] publicKey, long[] T, byte[] seedA, int seedAOffset) {
        int j = 0;
        ...
        System.arraycopy(seedA, seedAOffset, publicKey, 38400, 32);
    }

    static void decodePublicKey(int[] publicKey, byte[] seedA, int seedAOffset, byte[] publicKeyInput) {
        int j = 0;
        for (int i = 0; i < 10240; i += 16) {
            publicKey[i] = at(publicKeyInput, j, 0) & 1073741823;
            publicKey[i + 1] = ((at(publicKeyInput, j, 0) >>> 30) | (at(publicKeyInput, j, 1) << 2)) & 1073741823;
            publicKey[i + 2] = ((at(publicKeyInput, j, 1) >>> 28) | (at(publicKeyInput, j, 2) << 4)) & 1073741823;
            publicKey[i + 3] = ((at(publicKeyInput, j, 2) >>> 26) | (at(publicKeyInput, j, 3) << 6)) & 1073741823;
            publicKey[i + 4] = ((at(publicKeyInput, j, 3) >>> 24) | (at(publicKeyInput, j, 4) << 8)) & 1073741823;
            publicKey[i + 5] = ((at(publicKeyInput, j, 4) >>> 22) | (at(publicKeyInput, j, 5) << 10)) & 1073741823;
            publicKey[i + 6] = ((at(publicKeyInput, j, 5) >>> 20) | (at(publicKeyInput, j, 6) << 12)) & 1073741823;
            publicKey[i + 7] = ((at(publicKeyInput, j, 6) >>> 18) | (at(publicKeyInput, j, 7) << 14)) & 1073741823;
            publicKey[i + 8] = ((at(publicKeyInput, j, 7) >>> 16) | (at(publicKeyInput, j, 8) << 16)) & 1073741823;
            publicKey[i + 9] = ((at(publicKeyInput, j, 8) >>> 14) | (at(publicKeyInput, j, 9) << 18)) & 1073741823;
            publicKey[i + 10] = ((at(publicKeyInput, j, 9) >>> 12) | (at(publicKeyInput, j, 10) << 20)) & 1073741823;
            publicKey[i + 11] = ((at(publicKeyInput, j, 10) >>> 10) | (at(publicKeyInput, j, 11) << 22)) & 1073741823;
            publicKey[i + 12] = ((at(publicKeyInput, j, 11) >>> 8) | (at(publicKeyInput, j, 12) << 24)) & 1073741823;
            publicKey[i + 13] = ((at(publicKeyInput, j, 12) >>> 6) | (at(publicKeyInput, j, 13) << 26)) & 1073741823;
            publicKey[i + 14] = ((at(publicKeyInput, j, 13) >>> 4) | (at(publicKeyInput, j, 14) << 28)) & 1073741823;
            publicKey[i + 15] = (at(publicKeyInput, j, 14) >>> 2) & 1073741823;
            j += 15;
        }
        System.arraycopy(publicKeyInput, 38400, seedA, seedAOffset, 32);
    }

    private static boolean testZ(long[] Z) {
        for (int i = 0; i < 2048; i++) {
            if (Z[i] < -2096250 || Z[i] > 2096250) {
                return true;
            }
        }
        return false;
    ...
    static void encodeSignature(byte[] signature, int signatureOffset, byte[] C, int cOffset, long[] Z) {
        ...
        for (int i = 0; i < 1408; i += 11) {
            at(signature, i, 0, (int) ((Z[j + 0] & 4194303) | (Z[j + 1] << 22)));
            at(signature, i, 1, (int) (((Z[j + 1] >>> 10) & 4095) | (Z[j + 2] << 12)));
            at(signature, i, 2, (int) (((Z[j + 2] >>> 20) & 3) | ((Z[j + 3] & 4194303) << 2) | (Z[j + 4] << 24)));
            at(signature, i, 3, (int) (((Z[j + 4] >>> 8) & 16383) | (Z[j + 5] << 14)));
            at(signature, i, 4, (int) (((Z[j + 5] >>> 18) & 15) | ((Z[j + 6] & 4194303) << 4) | (Z[j + 7] << 26)));
            at(signature, i, 5, (int) (((Z[j + 7] >>> 6) & 65535) | (Z[j + 8] << 16)));
            at(signature, i, 6, (int) (((Z[j + 8] >>> 16) & 63) | ((Z[j + 9] & 4194303) << 6) | (Z[j + 10] << 28)));
            at(signature, i, 7, (int) (((Z[j + 10] >>> 4) & 262143) | (Z[j + 11] << 18)));
            at(signature, i, 8, (int) (((Z[j + 11] >>> 14) & 255) | ((Z[j + 12] & 4194303) << 8) | (Z[j + 13] << 30)));
            at(signature, i, 9, (int) (((Z[j + 13] >>> 2) & 1048575) | (Z[j + 14] << 20)));
            at(signature, i, 10, (int) (((Z[j + 14] >>> 12) & 1023) | (Z[j + 15] << 10)));
            j += 16;
        }
        System.arraycopy(C, cOffset, signature, signatureOffset + 5632, 32);
    }

    static void decodeSignature(byte[] C, long[] Z, byte[] signature, int signatureOffset) {
        int j = 0;
        for (int i = 0; i < 2048; i += 16) {
            ...
            Z[i] = (long) ((s0 << 10) >> 10);
            Z[i + 1] = (long) ((s0 >>> 22) | ((s1 << 20) >> 10));
            Z[i + 2] = (long) ((s1 >>> 12) | ((s2 << 30) >> 10));
            Z[i + 3] = (long) ((s2 << 8) >> 10);
            Z[i + 4] = (long) ((s2 >>> 24) | ((s3 << 18) >> 10));
            Z[i + 5] = (long) ((s3 >>> 14) | ((s4 << 28) >> 10));
            Z[i + 6] = (long) ((s4 << 6) >> 10);
            Z[i + 7] = (long) ((s4 >>> 26) | ((s5 << 16) >> 10));
            Z[i + 8] = (long) ((s5 >>> 16) | ((s6 << 26) >> 10));
            Z[i + 9] = (long) ((s6 << 4) >> 10);
            Z[i + 10] = (long) ((s6 >>> 28) | ((s7 << 14) >> 10));
            Z[i + 11] = (long) ((s7 >>> 18) | ((s8 << 24) >> 10));
            Z[i + 12] = (long) ((s8 << 2) >> 10);
            Z[i + 13] = (long) ((s8 >>> 30) | ((s9 << 12) >> 10));
            Z[i + 14] = (long) ((s9 >>> 20) | ((s10 << 22) >> 10));
            Z[i + 15] = (long) (s10 >> 10);
            j += 11;
        }
        System.arraycopy(signature, signatureOffset + 5632, C, 0, 32);
    }

    static void encodeC(int[] positionList, short[] signList, byte[] output, int outputOffset) {
        short domainSeparator;
        int count = 0;
        short[] C = new short[2048];
        byte[] randomness = new byte[168];
        short domainSeparator2 = (short) 1;
        HashUtils.customizableSecureHashAlgorithmKECCAK128Simple(randomness,
                0, (short)168, (short) 0, output, outputOffset, 32);
        Arrays.fill(C, (short) 0);
        int i = 0;
        while (i < 40) {
            if (count > 165) {
                domainSeparator = (short) (domainSeparator2 + 1);
                HashUtils.customizableSecureHashAlgorithmKECCAK128Simple(randomness, 0, 168, domainSeparator2, output, outputOffset, 32);
                count = 0;
            } else {
                domainSeparator = domainSeparator2;
            }
            int position = ((randomness[count] << 8) | (randomness[count + 1] & 255)) & 2047;
            if (C[position] == 0) {
            ...
            count += 3;
            domainSeparator2 = domainSeparator;
        }
    }

    private static void hashFunction(byte[] output, int outputOff, long[] v, byte[] hm, int hmOff) {
        byte[] T = new byte[10320];
        for (int k = 0; k < 5; k++) {
            int index = k * 2048;
            for (int i = 0; i < 2048; i++) {
                int temp = (int) v[index];
                int mask = (428072960 - temp) >> 31;
                int temp2 = ((temp - PARAM_Q) & mask) | ((mask ^ -1) & temp);
                int cL = temp2 & 16777215;
                int mask2 = (8388608 - cL) >> 31;
                index++;
                T[index] = (byte) ((temp2 - (((cL - 16777216) & mask2) | ((mask2 ^ -1) & cL))) >> 24);
            }
        }
        System.arraycopy(hm, hmOff, T, 10240, 80);
        HashUtils.secureHashAlgorithmKECCAK256(output, outputOff, 32, T, 0, T.length);
    }
    ...
    static void sample_y(long[] y, byte[] seed, int seedOffset, int nonce) {
        ...
        int pos = 0;
        int nblocks = 2048;
        byte[] buf = new byte[((BPLUS1BYTES * 2048) + 1)];
        int nbytes = BPLUS1BYTES;
        short dmsp2 = (short) (nonce << 8);
        short dmsp3 = (short) (dmsp2 + 1);
        HashUtils.customizableSecureHashAlgorithmKECCAK256Simple(buf, 0, nbytes * 2048, dmsp2, seed, seedOffset, 32);
        while (i < 2048) {
            if (pos >= nblocks * nbytes) {
                nblocks = NBLOCKS_SHAKE;
                dmsp = (short) (dmsp3 + 1);
                HashUtils.customizableSecureHashAlgorithmKECCAK256Simple(buf, 0, nbytes * 2048, dmsp3, seed, seedOffset, 32);
                pos = 0;
            } else {
                dmsp = dmsp3;
            }
            y[i] = (long) (lE24BitToInt(buf, pos) & maskb1);
            y[i] = y[i] - 2097151;
            if (y[i] != 2097152) {
                i++;
            }
    ...
    public static int at(byte[] bs, int base, int index) {
    ...
    static boolean test_correctness(long[] v, int vpos) {
        for (int i = 0; i < 2048; i++) {
            int mask = ((int) (428072960 - v[vpos + i])) >> 31;
            int val = (int) (((v[vpos + i] - 856145921) & ((long) mask)) | (v[vpos + i] & ((long) (mask ^ -1))));
            if (((((absolute(val) - 428072059) ^ -1) >>> 31) | (((absolute(val - ((((8388608 + val) - 1) >> 24) << 24)) - 8387707) ^ -1) >>> 31)) == 1) {
                return true;
            }
        }
        return false;
    ...
    private static boolean testRejection(long[] Z) {
        int valid = 0;
        for (int i = 0; i < 2048; i++) {
            valid = (int) (((long) valid) | (2096250 - absolute(Z[i])));
        }
        return (valid >>> 31) > 0;
    }
    ...
    private static boolean checkPolynomial(long[] polynomial, int polyOffset, int bound) {
        int sum = 0;
        int limit = 2048;
        long[] list = new long[2048];
        for (int j = 0; j < 2048; j++) {
            list[j] = (long) absolute((int) polynomial[polyOffset + j]);
        }
        for (int j2 = 0; j2 < 40; j2++) {
            for (int i = 0; i < limit - 1; i++) {
                long mask = (list[i + 1] - list[i]) >> 31;
                long temp = (list[i + 1] & mask) | (list[i] & (-1 ^ mask));
                list[i + 1] = (list[i] & mask) | (list[i + 1] & (-1 ^ mask));
                list[i] = temp;
            }
    ...
    static class Gaussian {
        ...
        static void sample_gauss_poly(int nonce, byte[] seed, int seedOffset, long[] poly, int polyOffset) {
            int dmsp = nonce << 8;
            byte[] samp = new byte[8192];
            int[] c = new int[4];
            for (int chunk = 0; chunk < 2048; chunk += 512) {
                dmsp++;
                HashUtils.customizableSecureHashAlgorithmKECCAK256Simple(samp, 0, 8192, (short) dmsp, seed, seedOffset, 32);
                for (int i = 0; i < 512; i++) {
                    poly[polyOffset + chunk + i] = 0;
                    for (int j = 1; j < CDT_ROWS; j++) {
                        int borrow = 0;
                        for (int k = 3; k >= 0; k--) {
                            c[k] = (int) (((long) (QTesla3p.at(samp, 0, (i * 4) + k) & Integer.MAX_VALUE)) - (cdt_v[(j * 4) + k] + ((long) borrow)));
                            borrow = c[k] >> 31;
                        }
                        int i2 = polyOffset + chunk + i;
                        poly[i2] = poly[i2] + ((long) ((borrow ^ -1) & 1));
                    }
                    int sign = QTesla3p.at(samp, 0, i * 4) >> 31;
                    poly[polyOffset + chunk + i] = (((long) sign) & (-poly[polyOffset + chunk + i])) | (((long) (sign ^ -1)) & poly[polyOffset + chunk + i]);
                }
            }
        }
    }
    ...
    static class QTesla3PPolynomial {
        ...
        static void poly_uniform(long[] a, byte[] seed, int seedOffset) {
            short dmsp;
            int pos = 0;
            int nblocks = QTesla3p.PARAM_GEN_A;
            byte[] buf = new byte[30240];
            short dmsp2 = (short) 1;
            HashUtils.customizableSecureHashAlgorithmKECCAK128Simple(buf, 0, 30240, (short) 0, seed, seedOffset, 32);
            int i = 0;
            while (i < 10240) {
                if (pos > (nblocks * 168) - 16) {
                    nblocks = 1;
                    dmsp = (short) (dmsp2 + 1);
                    HashUtils.customizableSecureHashAlgorithmKECCAK128Simple(buf, 0, 30240, dmsp2, seed, seedOffset, 32);
                    pos = 0;
                } else {
                    dmsp = dmsp2;
                }
                int val1 = Pack.littleEndianToInt(buf, pos) & 1073741823;
                int pos2 = pos + 4;
                int val2 = Pack.littleEndianToInt(buf, pos2) & 1073741823;
                int pos3 = pos2 + 4;
                int val3 = Pack.littleEndianToInt(buf, pos3) & 1073741823;
                int pos4 = pos3 + 4;
                int val4 = Pack.littleEndianToInt(buf, pos4) & 1073741823;
                pos = pos4 + 4;
                if (val1 < QTesla3p.PARAM_Q && i < 10240) {
                    a[i] = reduce(((long) val1) * 513161157);
                    i++;
                }
                if (val2 < QTesla3p.PARAM_Q && i < 10240) {
                    a[i] = reduce(((long) val2) * 513161157);
                    i++;
                }
                if (val3 < QTesla3p.PARAM_Q && i < 10240) {
                    a[i] = reduce(((long) val3) * 513161157);
                    i++;
                }
                if (val4 >= QTesla3p.PARAM_Q || i >= 10240) {
                    dmsp2 = dmsp;
                } else {
                    a[i] = reduce(((long) val4) * 513161157);
                    dmsp2 = dmsp;
                    i++;
                }
            }
        ...
        static long reduce(long a) {
            return (a + (((QTesla3p.PARAM_QINV * a) & 4294967295L) * 856145921)) >> 32;
        }

        static void ntt(long[] a, long[] w) {
            int j;
            int NumoProblems = 1024;
            int jTwiddle = 0;
            while (NumoProblems > 0) {
                for (int jFirst = 0; jFirst < 2048; jFirst = j + NumoProblems) {
                    jTwiddle++;
                    int W = (int) w[jTwiddle];
                    j = jFirst;
                    while (j < jFirst + NumoProblems) {
                        long temp = barr_reduce(reduce(((long) W) * a[j + NumoProblems]));
                        a[j + NumoProblems] = barr_reduce(a[j] + (1712291842 - temp));
                        a[j] = barr_reduce(a[j] + temp);
                        j++;
                    }
                }
                NumoProblems >>= 1;
        ...
        static long barr_reduce(long a) {
            return a - (856145921 * ((QTesla3p.PARAM_BARR_MULT * a) >> 32));
        }
        ...
        static void nttinv(long[] a, long[] w) {
            ...
            while (NumoProblems < 2048) {
                for (int jFirst = 0; jFirst < 2048; jFirst = j + NumoProblems) {
                    ...
                    while (j < jFirst + NumoProblems) {
                        long temp = a[j];
                        a[j] = barr_reduce(a[j + NumoProblems] + temp);
                        a[j + NumoProblems] = barr_reduce(reduce(((long) W) * ((1712291842 - a[j + NumoProblems]) + temp)));
                        j++;
                    }
                }
                NumoProblems *= 2;
        ...
        static void nttinv(long[] a, int aPos, long[] w) {
            ...
            while (NumoProblems < 2048) {
                for (int jFirst = 0; jFirst < 2048; jFirst = j + NumoProblems) {
                    ...
                    while (j < jFirst + NumoProblems) {
                        long temp = a[aPos + j];
                        a[aPos + j] = barr_reduce(a[aPos + j + NumoProblems] + temp);
                        a[aPos + j + NumoProblems] = barr_reduce(reduce(((long) W) * ((1712291842 - a[(aPos + j) + NumoProblems]) + temp)));
                        j++;
                    }
                }
                NumoProblems *= 2;
        ...
        static void poly_add_correct(long[] result, int rpos, long[] x, int xpos, long[] y, int ypos) {
            for (int i = 0; i < 2048; i++) {
                ...
                int i2 = rpos + i;
                result[i2] = result[i2] - 856145921;
                int i3 = rpos + i;
                result[i3] = result[i3] + ((result[rpos + i] >> 31) & 856145921);
            }
        }
        ...
        static void poly_sub_correct(int[] result, int[] x, int[] y) {
            for (int i = 0; i < 2048; i++) {
                result[i] = x[i] - y[i];
                result[i] = result[i] + ((result[i] >> 31) & QTesla3p.PARAM_Q);
            }
        }
        ...
        static void sparse_mul8(long[] prod, int ppos, byte[] s, int spos, int[] pos_list, short[] sign_list) {
            ...
            for (int i2 = 0; i2 < 40; i2++) {
                int pos = pos_list[i2];
                for (int j = 0; j < pos; j++) {
                    prod[ppos + j] = prod[ppos + j] - ((long) (sign_list[i2] * s[((spos + j) + 2048) - pos]));
                }
                for (int j2 = pos; j2 < 2048; j2++) {
                    prod[ppos + j2] = prod[ppos + j2] + ((long) (sign_list[i2] * s[(spos + j2) - pos]));
                }
            }
        }
        ...
        static void sparse_mul8(long[] prod, byte[] s, int[] pos_list, short[] sign_list) {
            ...
            for (int i2 = 0; i2 < 40; i2++) {
                int pos = pos_list[i2];
                for (int j = 0; j < pos; j++) {
                    prod[j] = prod[j] - ((long) (sign_list[i2] * s[(j + 2048) - pos]));
                }
                for (int j2 = pos; j2 < 2048; j2++) {
                    prod[j2] = prod[j2] + ((long) (sign_list[i2] * s[j2 - pos]));
                }
            }
        }
        ...
        static void sparse_mul16(int[] prod, int[] s, int[] pos_list, short[] sign_list) {
            ...
            for (int i2 = 0; i2 < 40; i2++) {
                int pos = pos_list[i2];
                for (int j = 0; j < pos; j++) {
                    prod[j] = prod[j] - (sign_list[i2] * s[(j + 2048) - pos]);
                }
                for (int j2 = pos; j2 < 2048; j2++) {
                    prod[j2] = prod[j2] + (sign_list[i2] * s[j2 - pos]);
                }
            }
        }
        ...
        static void sparse_mul32(int[] prod, int[] pk, int[] pos_list, short[] sign_list) {
            ...
            for (int i2 = 0; i2 < 40; i2++) {
                int pos = pos_list[i2];
                for (int j = 0; j < pos; j++) {
                    prod[j] = prod[j] - (sign_list[i2] * pk[(j + 2048) - pos]);
                }
                for (int j2 = pos; j2 < 2048; j2++) {
                    prod[j2] = prod[j2] + (sign_list[i2] * pk[j2 - pos]);
                }
            }
        }
        ...
        static void sparse_mul32(long[] prod, int ppos, int[] pk, int pkPos, int[] pos_list, short[] sign_list) {
            ...
            for (int i2 = 0; i2 < 40; i2++) {
                int pos = pos_list[i2];
                for (int j = 0; j < pos; j++) {
                    prod[ppos + j] = prod[ppos + j] - ((long) (sign_list[i2] * pk[((pkPos + j) + 2048) - pos]));
                }
                for (int j2 = pos; j2 < 2048; j2++) {
                    prod[ppos + j2] = prod[ppos + j2] + ((long) (sign_list[i2] * pk[(pkPos + j2) - pos]));
                }
            }
        }
    }
