...
public class ARIAEngine implements BlockCipher {
    ...
    public int processBlock(byte[] in, int inOff, byte[] out, int outOff) throws DataLengthException, IllegalStateException {
        if (this.roundKeys == null) {
            ...
            while (i < rounds) {
                ...
                i = i2 + 1;
                FE(z, this.roundKeys[i2]);
            }
            int i3 = i + 1;
            FO(z, this.roundKeys[i]);
            xor(z, this.roundKeys[i3]);
            SL2(z);
            xor(z, this.roundKeys[i3 + 1]);
    ...
    protected static byte[][] keySchedule(boolean forEncryption, byte[] K) {
        ...
        byte[][] rks = (byte[][]) Array.newInstance(Byte.TYPE, numRounds + 1, 16);
