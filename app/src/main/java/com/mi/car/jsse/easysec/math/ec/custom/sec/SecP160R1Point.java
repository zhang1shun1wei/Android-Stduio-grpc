...
public class SecP160R1Point extends ECPoint.AbstractFp {
    ...
    public ECPoint add(ECPoint b) {
        ...
        SecP160R1Field.subtract(S1, S2, t2);
        if (!Nat160.isZero(H)) {
            SecP160R1Field.square(H, t3);
            int[] G = Nat160.create();
            SecP160R1Field.multiply(t3, H, G);
            SecP160R1Field.multiply(t3, U1, t3);
            SecP160R1Field.negate(G, G);
            Nat160.mul(S1, G, tt1);
            SecP160R1Field.reduce32(Nat160.addBothTo(t3, t3, G), G);
            SecP160R1FieldElement X3 = new SecP160R1FieldElement(t4);
            SecP160R1Field.square(t2, X3.x);
            SecP160R1Field.subtract(X3.x, G, X3.x);
            SecP160R1FieldElement Y3 = new SecP160R1FieldElement(G);
            SecP160R1Field.subtract(t3, X3.x, Y3.x);
            SecP160R1Field.multiplyAddToExt(Y3.x, t2, tt1);
    ...
    public ECPoint twice() {
        ...
        SecP160R1Field.add(X1.x, Z1Squared, t2);
        SecP160R1Field.multiply(t2, t1, t2);
        SecP160R1Field.reduce32(Nat160.addBothTo(t2, t2, t2), t2);
        SecP160R1Field.multiply(Y1Squared, X1.x, Y1Squared);
        SecP160R1Field.reduce32(Nat.shiftUpBits(5, Y1Squared, 2, 0), Y1Squared);
        SecP160R1Field.reduce32(Nat.shiftUpBits(5, T, 3, 0, t1), t1);
        SecP160R1FieldElement X3 = new SecP160R1FieldElement(T);
        SecP160R1Field.square(t2, X3.x);
        SecP160R1Field.subtract(X3.x, Y1Squared, X3.x);
        SecP160R1Field.subtract(X3.x, Y1Squared, X3.x);
        SecP160R1FieldElement Y3 = new SecP160R1FieldElement(Y1Squared);
        SecP160R1Field.subtract(Y1Squared, X3.x, Y3.x);
        SecP160R1Field.multiply(Y3.x, t2, Y3.x);
        SecP160R1Field.subtract(Y3.x, t1, Y3.x);
        SecP160R1FieldElement Z3 = new SecP160R1FieldElement(t2);
